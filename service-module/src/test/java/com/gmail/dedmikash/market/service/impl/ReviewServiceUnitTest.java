...
public class ReviewServiceUnitTest {
    ...
    @Before
    public void init() {
        reviewService = new ReviewServiceImpl(reviewConverter, reviewRepository, userRepository);
    }

    @Test
    public void shouldReturnRightReviewDTOsListWhenGetReviewsBatch() {
        when(reviewRepository.findPage(1)).thenReturn(reviewList);
        ReviewDTO firstReviewDTO = new ReviewDTO(null, "text1", firstReview.getCreated(), true);
        ReviewDTO secondReviewDTO = new ReviewDTO(null, "text2", firstReview.getCreated(), false);
